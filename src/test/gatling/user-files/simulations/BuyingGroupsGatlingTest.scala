...
/**
 * Performance test for the BuyingGroups entity.
 */
class BuyingGroupsGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the BuyingGroups entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all buyingGroups")
            .get("/importer/api/buying-groups")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new buyingGroups")
            .post("/importer/api/buying-groups")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "buyingGroupName":"SAMPLE_TEXT"
                , "validFrom":"2020-01-01T00:00:00.000Z"
                , "validTo":"2020-01-01T00:00:00.000Z"
                }""")).asJSON
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_buyingGroups_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created buyingGroups")
                .get("/importer${new_buyingGroups_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created buyingGroups")
            .delete("/importer${new_buyingGroups_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
