...
/**
 * Performance test for the ColdRoomTemperatures entity.
 */
class ColdRoomTemperaturesGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the ColdRoomTemperatures entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all coldRoomTemperatures")
            .get("/importer/api/cold-room-temperatures")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new coldRoomTemperatures")
            .post("/importer/api/cold-room-temperatures")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_coldRoomTemperatures_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created coldRoomTemperatures")
                .get("/importer${new_coldRoomTemperatures_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created coldRoomTemperatures")
            .delete("/importer${new_coldRoomTemperatures_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
