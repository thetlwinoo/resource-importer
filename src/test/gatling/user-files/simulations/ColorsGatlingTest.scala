...
/**
 * Performance test for the Colors entity.
 */
class ColorsGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Colors entity")
        ...
        .repeat(2) {
            ...
            .get("/importer/api/colors")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new colors")
            .post("/importer/api/colors")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "colorName":"SAMPLE_TEXT"
                , "validFrom":"2020-01-01T00:00:00.000Z"
                , "validTo":"2020-01-01T00:00:00.000Z"
                }""")).asJSON
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_colors_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created colors")
                .get("/importer${new_colors_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created colors")
            .delete("/importer${new_colors_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
