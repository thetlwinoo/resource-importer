...
/**
 * Performance test for the CustomerCategories entity.
 */
class CustomerCategoriesGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the CustomerCategories entity")
        ...
        .repeat(2) {
            ...
            .get("/importer/api/customer-categories")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new customerCategories")
            .post("/importer/api/customer-categories")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_customerCategories_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created customerCategories")
                .get("/importer${new_customerCategories_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created customerCategories")
            .delete("/importer${new_customerCategories_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
