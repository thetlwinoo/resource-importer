...
/**
 * Performance test for the InvoiceLines entity.
 */
class InvoiceLinesGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the InvoiceLines entity")
        ...
        .repeat(2) {
            ...
            .get("/importer/api/invoice-lines")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new invoiceLines")
            .post("/importer/api/invoice-lines")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_invoiceLines_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created invoiceLines")
                .get("/importer${new_invoiceLines_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created invoiceLines")
            .delete("/importer${new_invoiceLines_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
