...
/**
 * Performance test for the OrderLines entity.
 */
class OrderLinesGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the OrderLines entity")
        ...
        .repeat(2) {
            ...
            .get("/importer/api/order-lines")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new orderLines")
            .post("/importer/api/order-lines")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_orderLines_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created orderLines")
                .get("/importer${new_orderLines_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created orderLines")
            .delete("/importer${new_orderLines_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
