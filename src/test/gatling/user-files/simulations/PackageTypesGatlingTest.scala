...
/**
 * Performance test for the PackageTypes entity.
 */
class PackageTypesGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the PackageTypes entity")
        ...
        .repeat(2) {
            ...
            .get("/importer/api/package-types")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new packageTypes")
            .post("/importer/api/package-types")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "packageTypeName":"SAMPLE_TEXT"
                , "validFrom":"2020-01-01T00:00:00.000Z"
                , "validTo":"2020-01-01T00:00:00.000Z"
                }""")).asJSON
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_packageTypes_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created packageTypes")
                .get("/importer${new_packageTypes_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created packageTypes")
            .delete("/importer${new_packageTypes_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
