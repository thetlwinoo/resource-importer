...
class PeopleGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the People entity")
        ...
        .repeat(2) {
            ...
            .get("/importer/api/people")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new people")
            .post("/importer/api/people")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "fullName":"SAMPLE_TEXT"
                , "preferredName":"SAMPLE_TEXT"
                , "searchName":"SAMPLE_TEXT"
                , "isPermittedToLogon":null
                , "logonName":"SAMPLE_TEXT"
                , "isExternalLogonProvider":null
                , "hashedPassword":"SAMPLE_TEXT"
                , "isSystemUser":null
                , "isEmployee":null
                , "isSalesPerson":null
                , "userPreferences":"SAMPLE_TEXT"
            ...
            .repeat(5) {
                ...
                .get("/importer${new_people_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created people")
            .delete("/importer${new_people_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
