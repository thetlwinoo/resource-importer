...
/**
 * Performance test for the ProductModel entity.
 */
class ProductModelGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the ProductModel entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all productModels")
            .get("/importer/api/product-models")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new productModel")
            .post("/importer/api/product-models")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
            ...
            .pause(10)
            .repeat(5) {
                exec(http("Get created productModel")
                .get("/importer${new_productModel_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created productModel")
            .delete("/importer${new_productModel_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
