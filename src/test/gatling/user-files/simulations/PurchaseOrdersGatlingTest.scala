...
/**
 * Performance test for the PurchaseOrders entity.
 */
class PurchaseOrdersGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the PurchaseOrders entity")
        ...
        .repeat(2) {
            ...
            .get("/importer/api/purchase-orders")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new purchaseOrders")
            .post("/importer/api/purchase-orders")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_purchaseOrders_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created purchaseOrders")
                .get("/importer${new_purchaseOrders_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created purchaseOrders")
            .delete("/importer${new_purchaseOrders_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
