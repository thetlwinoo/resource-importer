...
/**
 * Performance test for the SpecialDeals entity.
 */
class SpecialDealsGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the SpecialDeals entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all specialDeals")
            .get("/importer/api/special-deals")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new specialDeals")
            .post("/importer/api/special-deals")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
            ...
            .pause(10)
            .repeat(5) {
                exec(http("Get created specialDeals")
                .get("/importer${new_specialDeals_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created specialDeals")
            .delete("/importer${new_specialDeals_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
