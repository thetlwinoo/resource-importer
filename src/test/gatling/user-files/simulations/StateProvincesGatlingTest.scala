...
/**
 * Performance test for the StateProvinces entity.
 */
class StateProvincesGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the StateProvinces entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all stateProvinces")
            .get("/importer/api/state-provinces")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new stateProvinces")
            .post("/importer/api/state-provinces")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_stateProvinces_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created stateProvinces")
                .get("/importer${new_stateProvinces_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created stateProvinces")
            .delete("/importer${new_stateProvinces_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
