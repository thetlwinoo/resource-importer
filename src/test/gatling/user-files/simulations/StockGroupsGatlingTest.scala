...
/**
 * Performance test for the StockGroups entity.
 */
class StockGroupsGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the StockGroups entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all stockGroups")
            .get("/importer/api/stock-groups")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new stockGroups")
            .post("/importer/api/stock-groups")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "stockGroupName":"SAMPLE_TEXT"
                , "validFrom":"2020-01-01T00:00:00.000Z"
                , "validTo":"2020-01-01T00:00:00.000Z"
                }""")).asJSON
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_stockGroups_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created stockGroups")
                .get("/importer${new_stockGroups_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created stockGroups")
            .delete("/importer${new_stockGroups_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
