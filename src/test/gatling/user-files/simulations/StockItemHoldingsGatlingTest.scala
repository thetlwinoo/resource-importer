...
/**
 * Performance test for the StockItemHoldings entity.
 */
class StockItemHoldingsGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the StockItemHoldings entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all stockItemHoldings")
            .get("/importer/api/stock-item-holdings")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new stockItemHoldings")
            .post("/importer/api/stock-item-holdings")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "quantityOnHand":"0"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_stockItemHoldings_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created stockItemHoldings")
                .get("/importer${new_stockItemHoldings_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created stockItemHoldings")
            .delete("/importer${new_stockItemHoldings_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
