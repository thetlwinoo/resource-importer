...
/**
 * Performance test for the StockItems entity.
 */
class StockItemsGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the StockItems entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all stockItems")
            .get("/importer/api/stock-items")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new stockItems")
            .post("/importer/api/stock-items")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
            ...
            .pause(10)
            .repeat(5) {
                exec(http("Get created stockItems")
                .get("/importer${new_stockItems_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created stockItems")
            .delete("/importer${new_stockItems_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
