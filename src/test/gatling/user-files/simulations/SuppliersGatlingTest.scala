...
/**
 * Performance test for the Suppliers entity.
 */
class SuppliersGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Suppliers entity")
        ...
        .repeat(2) {
            ...
            .get("/importer/api/suppliers")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new suppliers")
            .post("/importer/api/suppliers")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                ...
                , "bankAccountCode":"SAMPLE_TEXT"
                , "bankAccountNumber":"SAMPLE_TEXT"
                , "bankInternationalCode":"SAMPLE_TEXT"
                , "paymentDays":"0"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_suppliers_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created suppliers")
                .get("/importer${new_suppliers_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created suppliers")
            .delete("/importer${new_suppliers_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
