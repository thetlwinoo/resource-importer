...
/**
 * Performance test for the UnitMeasure entity.
 */
class UnitMeasureGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the UnitMeasure entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all unitMeasures")
            .get("/importer/api/unit-measures")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new unitMeasure")
            .post("/importer/api/unit-measures")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
            ...
            .pause(10)
            .repeat(5) {
                exec(http("Get created unitMeasure")
                .get("/importer${new_unitMeasure_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created unitMeasure")
            .delete("/importer${new_unitMeasure_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
